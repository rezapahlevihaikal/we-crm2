                  ...
                  <form action="{{route('stages.store')}}" method="post" enctype="multipart/form-data">
                    ...
                    @method('POST')
                      <div class="form-group">
                        <label for="formGroupExampleInput2">Nama Stages</label>
                        <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="" name="nama_stage">
                      </div>
                        ...
                        <tbody>
                            @foreach($dataStage as $item)
                              <tr style="text-align: center">
                                <td><a href="{{route('stages.edit', $item->id)}}" title="">{{$item->nama_stage}}</a></td>
                                <td>{{$item->created_at}}</td>
                                <td>
                                  <form action="{{route('stages.destroy',$item->id)}}" method="POST">
                                    @csrf
                                    @method('post')
