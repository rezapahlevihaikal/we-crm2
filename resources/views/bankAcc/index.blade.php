            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form action="{{route('bankAcc.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                      <div class="form-group">
                        <label for="formGroupExampleInput2">Nama Bank</label>
                        <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="" name="nama_bank">
                      </div>
            ...
            <div class="card bg-secondary-default shadow">
                <div class="" style="padding:25px">
                    <table class="table table-bordered text-center" id="table-os">
                        ...
                        <tbody>
                            @foreach($dataBankAcc as $item)
                              <tr style="text-align: center">
                                <td><a href="{{route('bankAcc.edit', $item->id)}}" title="">{{$item->nama_bank}}</a></td>
                                <td>{{$item->created_at}}</td>
                                <td>
                                  <form action="{{route('bankAcc.destroy',$item->id)}}" method="POST">
                                    @csrf
                                    @method('post')
